...
// import { IconClear, IconStop, IconQueue } from "../../common/Icons";
    ...
    <div className={styles.page}>
      <div className={styles.topbar}>
        <div className={styles.topbarLeft}>
          <span className={styles.pageTitle}>Queue Consumer</span>
          {connected && <span className={styles.tagPurple}>● Consuming</span>}
        </div>
      ...
      <div className={styles.body}>
        <div className={styles.controlRow}>
          ...
          {!connected
            ? (
              <button className={styles.btnPrimary} onClick={() => onStart(queueName)}>
                <IconQueue color="#fff" size={13} /> Start
              </button>
            ) : (
              <button className={styles.btnDanger} onClick={onStop}>
                <IconStop color="#fff" size={13} /> Stop
              </button>
            )
          }
        ...
        {status && (
          <div className={`${styles.statusBar} ${connected ? styles.statusBarActive : ""}`}>
            {status}
          </div>
        )}
              ...
              <div className={styles.emptyState}>
                ...
                <p className={styles.emptyText}>
                  {connected
                    ...
                    : "Enter a queue name to start consuming"}
                </p>
              </div>
            )
            : messages.map((m) => (
              <div key={m.id} className={styles.msgCard}>
                <div className={styles.msgHeader}>
                  <span className={styles.queuePill}>{m.queue}</span>
